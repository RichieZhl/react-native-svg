...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (templateNode) {
        ...
        if ([templateNode class] == [RNSVGSymbol class]) {
            ...
            [templateNode renderTo:context rect:rect];
        }
    ...
    CGRect bounds = templateNode.clientRect;
...
- (RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    RNSVGNode const* templateNode = [self.svgView getDefinedTemplate:self.href];
    if (event) {
        self.active = NO;
    } else if (self.active) {
        return self;
    }
    RNSVGPlatformView const* hitChild = [templateNode hitTest:transformed withEvent:event];
    if (hitChild) {
        self.active = YES;
        return self;
    }
...
- (CGPathRef)getPath: (CGContextRef)context
{
    ...
    CGPathRef path = [templateNode getPath:context];
